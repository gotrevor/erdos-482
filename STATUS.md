...
**Stoll's binary-digits-of-759250125√2 (generalizes Graham–Pollak / Erdős #482), formalized in Lean 4.** · **Build**: 🟢 green (8296 jobs) · **Updated**: lap 2026-06-14 (`st06` branch, HEAD `5cd552f`) (**#482 COMPLETE & axiom-clean on `main`; St06 fun-extension on `st06` ALL main theorems done; cubic AND quartic a.e.-`W` self-referential impossibilities COMPLETE & axiom-clean. LIVE: GENERAL degree-`d` (`α=2^{1/d}`) — the algebraic obstruction, faithfulness recovery (cubic & quartic), AND the full analytic bridge (orbit-coordinate form `dGpd` + `continuousAt_dGpd`, `Tᵈ` density `ae_W_dTorus_orbit_dense`, realization `orbitF_realizeR`) are ALL COMPLETE & axiom-clean. ONLY the final geometric headline assembly remains (nonzero-coord realization + threshold chain)**)
