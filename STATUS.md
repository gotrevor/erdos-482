...
**Stoll's binary-digits-of-759250125√2 (generalizes Graham–Pollak / Erdős #482), formalized in Lean 4.** · **Build**: 🟢 green (8294 jobs) · **Updated**: lap 2026-06-14 (`st06` branch, HEAD `944a8d9`) (**#482 COMPLETE & axiom-clean on `main`; St06 fun-extension on `st06` ALL main theorems done; cubic AND quartic a.e.-`W` self-referential impossibilities COMPLETE & axiom-clean. LIVE: GENERAL degree-`d` (`α=2^{1/d}`) — the algebraic + abstract-geometric obstruction SKELETON is now COMPLETE & axiom-clean (`RpowLinIndep`/`RpowWindow`/`GeneralDefect`), incl. the abstract crux `exists_partial_defect_outside_window` + faithfulness recovery of the cubic & quartic from the general engine (`GeneralDefectCheck`); remaining = analytic `Tᵈ`-assembly only (engine already degree-agnostic)**)
