...
* the **original** Graham–Pollak track (`Erdos482.u`, `Erdos482.graham_pollak`), an elementary
...
theorem gv_sqrt2_eq_u (a b ε : ℝ) (ha : a = Real.sqrt 2) (hb : b = Real.sqrt 2) (hε : ε = 1 / 2)
    (m : ℕ) : gv a b ε m = (Erdos482.u m : ℤ) := by
  subst ha hb hε
  induction m with
  | zero => rfl
  | succ k ih =>
    have hnn : (0 : ℝ) ≤ Real.sqrt 2 * ((Erdos482.u k : ℝ) + 1 / 2) := by positivity
    rw [gv_succ]
    ...
    rw [show Erdos482.u (k + 1) = ⌊Real.sqrt 2 * ((Erdos482.u k : ℝ) + 1 / 2)⌋₊ from rfl,
...
`Erdos482.graham_pollak` (which proves the companion odd-index identity by a direct induction) —
two independent routes to "√2's binary expansion is encoded in `u`". -/
theorem gp_sqrt2_digits_via_general (n : ℕ) (hn : 1 ≤ n) :
    (Erdos482.u (2 * n) : ℤ) - 2 * (Erdos482.u (2 * n - 2) : ℤ)
